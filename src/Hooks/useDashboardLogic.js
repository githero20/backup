...
function useDashboardLogic(reload) {
  ...
  useEffect(() => {
    ...
    if (token)
      setupDashBoard().then((data) =>
        setState({ ...state, ...data, showLoader: false })
      );
  }, [reload]);
  ...
  async function setupDashBoard() {
    ...
    adModalController();
    try {
      const [userInfoRes, centralVaultIntRes, lockedIntRes, backUpRes] =
        await Promise.all([
  ...
  const lockedSavingsInterest = lockedSavingsData
    ? parseFloat(lockedSavingsData).toFixed(2)
    : 0;
  const vaultInterest = centralVaultData
    ? parseFloat(centralVaultData).toFixed(2)
    : 0;
