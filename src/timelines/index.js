...
export const play = (pathname, node, appears) => {
    ...
    let timeline;
    if (pathname === '/' || pathname === '/faq') timeline = getHomeTimeline(node, delay);
    else if (loginUrls.includes(pathname)) timeline = getLoginTimeline(node, delay);
    else timeline = getDashboardTimeline(node, delay);
    if (timeline) window.loadPromise.then(() => requestAnimationFrame(() => timeline.play()))
