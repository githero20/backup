...
export function getTotal(transactions) {
  if (transactions) {
    const sum = transactions.reduce((a, b) => ({
...
export function getTotalSteadySave(transactions) {
  if (transactions && transactions.length > 0) {
    ...
    credits = credits.reduce((a, b) => ({
      amount: parseInt(a.start_amount) + parseInt(b.start_amount),
    }));
...
export function getTotalSteadySaveDebit(transactions) {
  if (transactions) {
    if (transactions.length > 0) {
      ...
      debits = debits.reduce((a, b) => ({
        amount: parseInt(a.amount) + parseInt(b.amount),
      }));
      return debits.amount;
    } else {
      let sum = transactions.reduce((a, b) => ({
        amount: parseInt(a.amount) + parseInt(b.amount),
      }));
      return sum.amount;
    }
...
export function capitalize(value) {
  ...
  if (words.length > 1) {
    return words
      .map((content) => content.charAt(0).toUpperCase() + content.slice(1))
      .join(" ");
  } else {
  ...
  if (transactions) {
    if (transactions.length) {
      let data = transactions.filter(
        (content) => content.status == "success" || content == "failed"
      );
      return data;
    }
  }
...
export function getCompletedGoals(transactions) {
  ...
  if (transactions.length > 0) {
    let CompletedGoals = transactions.filter((content) => {
      return (
        (moment(content.end_date).format("YYYY-MM-DD") < now &&
...
export function isGoalCompleted(goal) {
  ...
  if (
    goal &&
    moment(goal.end_date).format("YYYY-MM-DD") < now &&
...
export const passwordValidator = new SimpleReactValidator({
  messages: {
    ...
    password:
      "Password combination must have a lowercase letter, uppercase letter, number, special character and must be a minimum of 8 characters",
  },
  validators: {
    password: {
      // name the rule
      message:
        "The :attribute must be a strong password and must have :values.",
  ...
  validators: {
    ...
    emailPhone: {
      ...
      rule: (val, params, validator) => {
        return validator.helpers.testRegex(
          val,
          /^([a-zcA-Z0-9_\.\-])+\@(([a-zA-Z0-9\-])+\.)+([a-zA-Z0-9]{2,4})|^[0]\d{10}$/
        );
      },
      messageReplace: (message, params) =>
        message.replace(":values", this.helpers.toSentence(params)), // optional
...
export function amountFormatter(cell, row) {
  return (
    <p
      style={{ minWidth: "100px" }}
      className={row.type === "credit" ? "text-green" : "text-red"}
    >
      {row.type === "credit" ? "+" : "-"}
    ...
    <div className="d-flex flex-column">
      ...
      <small>
        ...
        {row.last_amount != null
          ? `₦ ${formatNumber(parseFloat(row.last_amount).toFixed(2))}`
          : "N/A"}
      </small>
      <small className="text-muted">Date</small>
  ...
  return (
    <div className="d-flex flex-column">
      ...
      <small className="text-muted">
        {" "}
        {row.balance != null
          ? `₦ ${formatNumber(parseFloat(row.balance).toFixed(2))}`
          : "N/A"}
      </small>
      <small> {moment(row.created_at).format("Do MMM YY")}</small>
    </div>
...
export function moneyFormatter(cell, row) {
  return (
    <p style={{ minWidth: "150px" }} className={"text-green"}>
      {" "}
      {cell != null
        ? `+ ₦ ${formatNumber(parseFloat(cell).toFixed(2))}`
        : "N/A"}
  ...
  if (window.innerWidth < 600) {
    ...
    return (
      <div className="d-flex flex-column">
        ...
        <p style={{ minWidth: "100px" }} className={"text-gray"}>
          {" "}
          {cell != null
            ? `₦ ${formatNumber(parseFloat(cell).toFixed(2))}`
            : "N/A"}
...
export function amountCurrentStatusFormatter(cell, row) {
  return (
    <div className="d-flex flex-column">
      <p style={{ minWidth: "100px" }} className={"text-green"}>
        {" "}
        {cell != null
          ? `+ ₦ ${formatNumber(parseFloat(cell).toFixed(2))}`
          : "N/A"}
      ...
      <label
        className={
          row.status == "success"
            ? "bg-light-green text-center round px-1 "
            : "bg-light-red text-center px-1 round "
        }
      >
        {row.status}
      </label>
      ...
      <small>
        {row.current_amount != null
          ? `+ ₦ ${formatNumber(parseFloat(row.current_amount).toFixed(2))}`
          : "N/A"}
      </small>
    </div>
  );
...
export function toastReloadMessage(status, context, callback) {
  ...
  const message = (
    <Fragment>
      ...
      <a href="#" className="retry dark-link" onClick={() => callback()}>
        Try Again
      </a>
    </Fragment>
  );
...
export function calcPenalty(balance, penalty) {
  return Number(
    Number(balance).toFixed(2) * (Number(penalty).toFixed(2) / 100)
  ).toFixed(2);
}
...
export function steadyStatusFormatter(cell, row) {
  ...
  if (parseInt(row.is_pause)) {
    ...
    return (
      <button className={"btn btn-sm round btn-success"}>Completed</button>
    );
  } else {
...
export function statusFormatter(cell) {
  return (
    <label
      className={
        cell === "success"
          ? "bg-light-green px-2 sm-pd text-capitalize"
          : "bg-light-red px-2 sm-pd text-capitalize"
      }
    >
      {cell}
    </label>
...
export function amountInterestFormatter(cell, row) {
  return (
    <div className="d-flex flex-column">
      <p style={{ minWidth: "100px" }} className={"text-info"}>
        {" "}
        {row.amount != null
          ? `₦ ${formatNumber(parseFloat(row.amount).toFixed(2))}`
          : "N/A"}
      </p>
...
export function viewFormatter(cell) {
  return (
    <button className={"btn round btn-sm btn-blue-btn"}>View History</button>
  );
}
...
export function actionFormatter(cell, row, rowIndex, { trans }) {
  if (rowIndex > 0) {
    ...
    if (
      moment(latestDate).isBefore(tommorrow) &&
      moment(latestDate).isSame(endDate) &&
      rowIndex === 0
    ) {
      return (
        <button
          name="convert-btn"
          className={"btn btn-sm round btn-sm btn-danger"}
        >
          Convert
        </button>
      );
      ...
      row.end_date != null &&
      moment(row.end_date).isBefore(tommorrow)
    ) {
      return (
        <button disabled={true} className={"btn round btn-sm btn-secondary"}>
          Disabled
        </button>
      );
    } else if (
      ...
      return (
        <button className={"btn round btn-sm btn-secondary"}>
          Quick Actions
        </button>
      );
    }
...
export function detailFormatter(cell) {
  return (
    <button className={"btn round btn-sm btn-blue-btn"}>View Details</button>
  );
}
...
export function balanceFormatter(cell) {
  return (
    <label style={{ minWidth: "100px" }} className={"text-info"}>
      {cell != null ? `₦ ${formatNumber(parseFloat(cell).toFixed(2))}` : "N/A"}
    </label>
  );
}
...
export function pointFormatter(cell, row) {
  if (row.point_type === "referral_type") {
    ...
    return (
      <label style={{ minWidth: "100px" }} className={"text-info"}>{`0`}</label>
    );
  }
}
...
export function bonusAmountFormatter(cell, row) {
  if (row.point_type === "referral_bonus_centralvault") {
    return (
      <label style={{ minWidth: "100px" }} className={"text-info"}>
        {cell != null ? `₦ ${formatNumber(cell)}` : "N/A"}
      </label>
    );
  } else {
    return (
...
export function sourceTypeFormatter(cell) {
  return (
    <label style={{ minWidth: "100px" }} className={"text-info"}>
      {cell != null ? `₦ ${cell.data.name}` : "N/A"}
    </label>
  );
}

export function lockedStatusFormatter(cell) {
  return _isDateAfterToday(cell) ? (
...
export function handlePinConcatenation(
  name,
  event,
  context = this,
  callback = null
...
export function validateSteadySaveAmount(frequency, contribution, context) {
  if (
    frequency == APP_FREQUENCY.daily &&
    Number(contribution) < AMOUNT_LIMITS.minSteadySaveDaily
  ) {
    toastMessage(
      `Thec mimimum amount for ${
        APP_FREQUENCY.daily
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.minSteadySaveDaily)}`,
      "error",
      context
    );
    return false;
  } else if (
    frequency == APP_FREQUENCY.monthly &&
    Number(contribution) < AMOUNT_LIMITS.minSteadySaveMonthly
  ) {
    toastMessage(
      `The mimimum amount for ${
        APP_FREQUENCY.monthly
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.minSteadySaveMonthly)}`,
      "error",
      context
    );
    return false;
  } else if (
    frequency == APP_FREQUENCY.weekly &&
    Number(contribution) < AMOUNT_LIMITS.minBackUpGoalWeekly
  ) {
    toastMessage(
      `The mimimum amount for ${
        APP_FREQUENCY.weekly
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.minBackUpGoalWeekly)}`,
      "error",
      context
    );
    return false;
    ...
    frequency == APP_FREQUENCY.daily &&
    Number(contribution) > AMOUNT_LIMITS.maxSteadySaveDaily
  ) {
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.daily
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.maxSteadySaveDaily)}`,
      "error",
      context
    );
    return false;
    ...
    frequency == APP_FREQUENCY.monthly &&
    Number(contribution) > AMOUNT_LIMITS.maxSteadySaveMonthly
  ) {
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.monthly
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.maxSteadySaveMonthly)}`,
      "error",
      context
    );
    return false;
    ...
    frequency == APP_FREQUENCY.weekly &&
    Number(contribution) > AMOUNT_LIMITS.maxBackUpGoalWeekly
  ) {
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.weekly
      } steady save is ₦ ${formatNumber(AMOUNT_LIMITS.maxBackUpGoalWeekly)}`,
      "error",
      context
    );
    return false;
...
export function validateBackupGoalAmount(frequency, contribution, context) {
  if (
    frequency === APP_FREQUENCY.daily &&
    Number(contribution) < AMOUNT_LIMITS.minBackUpGoalDaily
  ) {
    toastMessage(
      `The mimimum amount for ${
        APP_FREQUENCY.daily
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.minBackUpGoalDaily)}`,
      "error",
      context
    );
    return false;
    ...
    frequency === APP_FREQUENCY.monthly &&
    Number(contribution) < AMOUNT_LIMITS.minBackUpGoalMonthly
  ) {
    toastMessage(
      `The mimimum amount for ${
        APP_FREQUENCY.monthly
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.minBackUpGoalMonthly)}`,
      "error",
      context
    );
    return false;
    ...
    toastMessage(
      `The mimimum amount for ${
        APP_FREQUENCY.weekly
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.minBackUpGoalWeekly)}`,
      "error",
      context
    );
    return false;
    ...
    frequency === APP_FREQUENCY.daily &&
    Number(contribution) > AMOUNT_LIMITS.maxBackUpGoalDaily
  ) {
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.daily
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.maxBackUpGoalDaily)}`,
      "error",
      context
    );
    return false;
    ...
    frequency === APP_FREQUENCY.monthly &&
    Number(contribution) > AMOUNT_LIMITS.maxBackUpGoalMonthly
  ) {
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.monthly
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.maxBackUpGoalMonthly)}`,
      "error",
      context
    );
    return false;
    ...
    toastMessage(
      `The maximum amount for ${
        APP_FREQUENCY.weekly
      } backup goals is ₦ ${formatNumber(AMOUNT_LIMITS.maxBackUpGoalWeekly)}`,
      "error",
      context
    );
    return false;
