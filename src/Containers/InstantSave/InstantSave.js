...
class InstantSave extends Component {
  ...
  getTotalInstantSave(transactions) {
    if (transactions) {
      //filter credits
      let instantSaves = transactions.filter(
        (content) => content.status === "success" && content.type === "credit"
      );
      //get sum of credits
      const sum = instantSaves.reduce((a, b) => ({
  ...
  handleTransactions = (state, res) => {
    ...
    if (state) {
      ...
      if (res.data.data.length > 0) {
        transactions = res.data.data.filter(
          (content) => content.status === "success" && content.type === "credit"
        );
  ...
  loadInstantSaveTable = (status, payload) => {
    ...
    if (status) {
      if (payload) {
        let transactions = payload.data.data.transactions.data.filter(
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        render: (value, record) => (
          <div>
            <button className="btn btn-sm round btn-primary">
              {record.type}
            </button>
          </div>
        ),
        ...
        render: (value, record) => (
          <div>
            {record.status === "success" ? (
              <button className="btn btn-sm round btn-success">
                {record.status}
              </button>
            ) : (
              <button className="btn btn-sm round btn-danger">
                {record.status}
              </button>
            )}
          </div>
            ...
            <div className="content-body">
              <div className="row">
                <div className="col-lg-5 col-12 order-lg-8">
                  <div
                    className={"descriptive-info mt-md-3 mt-0 mb-3 px-2 py-1"}
                  >
                    <p>
                      Start saving as soon as you earn an income or when you get
                      some loose cash. Make an instant transfer to your Backup
                      Cash account and earn 8% interest.
                    </p>
                  </div>
                </div>
                <div className="col-lg-4 col-12 order-lg-1">
                  <h3 className="gray-header-text fs-mb-1 mb-2 ">
                    Instant Save <span className="dot">.</span> Summary
                  </h3>
                  <InstantSaveCard
                ...
                <div className="col-lg-3 col-12 order-lg-5">
                  <h3 className="gray-header-text fs-mb-1 mb-2 mt-7px">
                    Quick Actions
                  </h3>
                    ...
                    <span className="mb-details-container ">
                      ...
                      <div className=" d-inline-block">
                        <strong className="dark-brown font-size-1-16">
                          <span>₦</span>{" "}
                          {formatNumber(
                            parseFloat(this.state.totalInstantSave).toFixed(2)
                          )}
                        </strong>
